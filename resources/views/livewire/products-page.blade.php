            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:border-gray-900 dark:bg-gray-900">
                        ...
                        <ul>
                            @foreach ($categories as $cate)
                                <li class="mb-4" wire:key='{{ $cate->id }}'>
                                    <label for="{{ $cate->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" wire:model.live="selected_categories"
                                            id="{{ $cate->slug }}" value="{{ $cate->id }}" class="w-4 h-4 mr-2">
                                        <span class="text-lg">{{ $cate->name }}</span>
                                    </label>
                                </li>
                            @endforeach
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold">{{ Number::currency($price, 'VND') }}</div>
                            <input type="range" wire:model.live="price"
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="1000000000" value="100000" step="100000">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">
                                    {{ Number::currency(1000, 'VND') }}</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">
                                    {{ Number::currency(1000000000, 'VND') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key='{{ $item->id }}'>
                                <div class="border border-gray-300 dark:border-gray-700">
                                    ...
                                    <div class="p-3 ">
                                        ...
                                        <p class="text-lg ">
                                            <span
                                                class="text-green-600 dark:text-green-600">{{ Number::currency($item->price, 'VND') }}</span>
                                        </p>
                                    </div>
